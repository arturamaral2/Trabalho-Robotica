...
pds(t) = [0.050*sin(wn*t)+0.428 0.500 0.050*cos(wn*t)+0.569];
%pds(t) = [0.050*sin(wn*t)+0.428 0.2 0.050*cos(wn*t)+0.669];
%pds(t) = [0.020*(sin(wn*t)+sin(4*wn*t))+0.428 0.500 0.020*(cos(wn*t)+cos(4*wn*t))+0.569];
...
while (norm(e) > epsilon) % Critério de parada
    ...
    k0=1;
    for d = 1:7
    qavg(d)= ((robo1.qlim(d,1)+robo1.qlim(d,2))/2);    
    den=((robo1.qlim(d,2)^2)-2*(robo1.qlim(d,2)*robo1.qlim(d,1))+(robo1.qlim(d,1)^2)); 
    deriv= 2*q(d)-2*qavg(d);
    fobj(d)=((-1/(14*den))*deriv)
    end
    u = (pinv(J)*(lambda*e))+((eye(7)-(pinv(J)*J))*k0*fobj'); % Lei de controle
...
for j = 1:1:34
    ...
    u = (pinv(J)*(pddot'+lambda*e))+((eye(7)-(pinv(J)*J))*k0*fobj'); % Lei de controle
    ...
    tf_robo =  @(t,qi) [u(1);u(2);u(3);u(4);u(5);u(6);u(7)];
    [~, qi] = ode45(tf_robo, 0:tf:tf, zeros(1,7));
